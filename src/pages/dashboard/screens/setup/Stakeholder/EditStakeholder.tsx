...
interface UpdateStakeholderModalProps {
  open: boolean;
  onClose: () => void;
  onSuccess?: () => void;
...
const UpdateStakeholderModal: React.FC<UpdateStakeholderModalProps> = ({
  open,
  onClose,
  onSuccess,
  stakeholder,
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      if (response.status === 'ok') {
        ...
        const errorMsg = response?.response?.data?.msg || response?.message || 'Failed to update stakeholder';
        toast.error(errorMsg, { id: loadingToast });
      }
    } catch (error: any) {
      console.error('Form submission error:', error);
      toast.error(error?.message || 'An error occurred while updating the stakeholder', { id: loadingToast });
    } finally {
